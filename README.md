...
- [v0.7.7 release notes](docs/release-notes/RELEASE_NOTES_v0.7.7.txt)
...
- The later `v0.7.7` follow-up resolved the `local-large-gpu` runtime-budget failure on the focused Brooklands control, so the lane is now viable on Rig1 for that control. It still remains a non-default recommendation for recurring Brooklands-sensitive work because `local-medium-gpu` already preserves `Brooklands` and remains much faster on Rig1.
