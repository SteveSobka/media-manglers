...
- [ ] Fix the repeated estimate-stage warning that still appears during benchmark planning
  Status: repo-only follow-up
  Current note: The 2026-04-17 German-to-English benchmark completed successfully in all five lanes, but every lane console log still reported `Estimate step failed: The term 'if' is not recognized as the name of a cmdlet...` before continuing. The adaptive Local Whisper runtime plans still resolved and the packages still validated, so this is a non-fatal bug, but it should be fixed before treating the estimate stage as fully trustworthy.
