...
- [ ] Issue #34: Investigate Brooklands source-transcript recall weakness in local benchmark evidence
  ...
  Current note: PR `#33` fixed the translation-side `Brooklands -> Brooklyn` corruption in the affected Hybrid benchmark lanes, and the issue-34 follow-up remains intentionally separate after the `v0.7.5` release. Preserved evidence plus the focused Rig1 rerun at `C:\DATA\TEMP\CODEX\mm-brooklands-source-recall-20260420` showed a real raw/source transcript substitution problem on the local medium lanes instead (`Brooklyns` on GPU and `Brooklynz` on CPU), so the warning was source-side rather than a reopened Hybrid translation defect. The current `wip/0.7.6-brooklands-source-transcript-recall-2026-04-20` branch now applies a benchmark-scoped Local Whisper initial prompt derived from manifest `expected_named_entities`; the focused rerun at `C:\DATA\TEMP\CODEX\mm-brooklands-source-recall-fix-20260420` shows literal `Brooklands` restored on both `local-medium-gpu` and `local-medium-cpu`, with the Hybrid control lane still `accepted` and `brooklands_to_brooklyn_flag=False`. The remaining `local-large-gpu` runtime-budget caveat is now split into issue `#38`. Keep issue `#34` open until the branch is reviewed and merged.

- [ ] Issue #38: Investigate local-large-gpu adaptive runtime budget for Brooklands benchmark runs
  Status: open follow-up
  Current note: The focused issue-34 comparison root at `C:\DATA\TEMP\CODEX\mm-brooklands-source-recall-20260420` showed that `local-large-gpu` did not finish cleanly on the Brooklands control, but that failure was runtime-budget / lane-viability work rather than the same source-transcript recall defect fixed on the `v0.7.6` branch. This follow-up should determine whether the large lane needs runtime-plan tuning, watchdog adjustment, or a narrower recommendation boundary on Rig1.
...
- [x] Issue #36: Avoid unnecessary OpenAI translation cost for English-to-English AI/Hybrid runs
  ...
  Current note: PR `#37` merged the English no-op translation fix into `main`, and GitHub release `v0.7.5` is now the current release surface. `AI Private` still uses OpenAI transcription when that mode explicitly requires it, but English-source to English-target translation now resolves to `Original transcript copy (no OpenAI call)` instead of paid OpenAI text translation. The merged Rig1 packaged proof at `C:\DATA\TEMP\CODEX\v075-postmerge-ai-noop-20260420` confirmed `transcription_provider = OpenAI`, `transcription_model = whisper-1`, `translation_provider_name = Original transcript copy`, `openai_translation_summary = OpenAI Translation: not used for this file; source already English, original transcript copied`, and `estimated_openai_text_cost_usd = 0.000000`. Issue `#34` remains open as the separate Brooklands source-transcript recall follow-up, and DevBox CPU was explicitly not treated as a blocker for this patch.
